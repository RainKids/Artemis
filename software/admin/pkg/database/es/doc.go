...
// Create 创建
func (cli *Client) Create(ctx context.Context, index, id, routing string, doc interface{}) (string, error) {
	var (
		res *elastic.IndexResponse
		err error
	)
	indexService := cli.Client.Index().Index(index)
	if len(id) > 0 {
		indexService.Id(id)
	}
	if len(routing) > 0 {
	...
	res, err = indexService.BodyJson(doc).Refresh(DefaultRefresh).Do(ctx)
	if err != nil {
...
func (c *Client) Update(ctx context.Context, indexName, id, routing string, update interface{}) error {
...
func (c *Client) UpdateRefresh(ctx context.Context, indexName, id, routing string, update interface{}) error {
